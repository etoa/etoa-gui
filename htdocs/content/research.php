	...
	if ($bl->getLevel(TECH_BUILDING_ID) > 0) {
		...
		if ($bid > 0) {
			...
			if (mysql_num_rows($res)>0) {
				...
				$bc['time'] = (array_sum($bc)) / GLOBAL_TIME * $config->getFloat('res_build_time') * $time_boni_factor;
				$bc['time'] *= $bonus;
			...
			if ($bid > 0)
			{
				...
				if ($arr)
				{
					...
					$btime = ($bc['metal']+$bc['crystal']+$bc['plastic']+$bc['fuel']+$bc['food']) / GLOBAL_TIME * $config->getFloat('res_build_time') * $time_boni_factor;
					$btime *= $bonus * $cu->specialist->researchTime;

					//Nächste Stufe
					$btimen = ($bcn['metal']+$bcn['crystal']+$bcn['plastic']+$bcn['fuel']+$bcn['food']) / GLOBAL_TIME * $config->getFloat('res_build_time') * $time_boni_factor;
