			      ...
			      if(isset($fleet[$ship_id]))
			      {
    				...
    				// TODO: Überprüfen
						...
						if ($build_cnt>0)
						{
							...
							/ GLOBAL_TIME * $config->getFloat('ship_build_time')
							* $time_boni_factor
					...
					if (count($ships) > 0)
					{
						...
						foreach ($ships as $data)
						{
							...
							if ($build_ship == 1 && $data['ship_cat_id'] == $cat_id)
							{
								...
								// Bauzeit berechnen
								$btime = ($data['ship_costs_metal'] + $data['ship_costs_crystal'] + $data['ship_costs_plastic'] + $data['ship_costs_fuel'] + $data['ship_costs_food']) / GLOBAL_TIME * $config->getFloat('ship_build_time') * $time_boni_factor * $cu->specialist->shipTime;
								$btime_min = $btime * (0.1 - ($gen_tech_level / 100));
