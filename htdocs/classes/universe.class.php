    ...
    class Universe
    {
        ...
        */
        static function create($mapImage="",$mapPrecision=95)
        {
            ...
            $perc_solsys = $config->getInt('space_percent_solsys');
            $perc_asteroids = $config->getInt('space_percent_asteroids');
            $perc_nebulas = $config->getInt('space_percent_nebulas');
            $perc_wormholes = $config->getInt('space_percent_wormholes');
        ...
        private static function createStarSystem($cell_id, $id=-1)
        {
            ...
            // The planets
            $np = mt_rand($config->param1Int('num_planets'),$config->param2Int('num_planets'));
            for ($cnp=1;$cnp<=$np;$cnp++)
            {
                $r = mt_rand(0,100);
                if ($r <= $config->getInt('solsys_percent_planet'))
                {
                    self::createPlanet($cell_id,$cnp,$np);
                }
                elseif ($r <= $config->getInt('solsys_percent_planet') + $config->getInt('solsys_percent_asteroids'))
                {
        ...
        private static function createPlanet($cell_id,$pos,$np)
        {
            ...
            $planet_temp_min = $config->param1Int('planet_temp');
            $planet_temp_max = $config->param2Int('planet_temp');
            $planet_temp_diff = $config->getInt('planet_temp');
            ...
            $img_nr = $pt."_".mt_rand(1,$config->getInt('num_planet_images'));
            $fields = mt_rand($config->param1Int('planet_fields'),$config->param2Int('planet_fields'));
        ...
        private static function createAsteroids($cell_id,$pos=0)
        {
            ...
            $asteroid_metal = mt_rand($config->param1Int('asteroid_ress'),$config->param2Int('asteroid_ress'));
            $asteroid_crystal = mt_rand($config->param1Int('asteroid_ress'),$config->param2Int('asteroid_ress'));
            $asteroid_plastic = mt_rand($config->param1Int('asteroid_ress'),$config->param2Int('asteroid_ress'));
            $sql = "
                INSERT INTO
                    asteroids
                (
        ...
        private static function createNebula($cell_id,$pos=0)
        {
            ...
            $nebula_ress = mt_rand($config->param1Int('nebula_ress'),$config->param2Int('nebula_ress'));
            $sql = "
                INSERT INTO
                    nebulas
                (
        ...
        private static function createWormhole($cell_id,$pos=0)
        {
            ...
            $persistent_wormholes_ratio = $config->getInt('persistent_wormholes_ratio');
            $sql = "
                INSERT INTO
                    entities
                (
