...
function listUsers(
    ...
    if ($request->request->has('new_submit')) {
        if ($request->request->get('user_nick') != "") {
            ...
            Log::add(LogFacility::ADMIN, Log::INFO, "Der Administrator " . $cu->nick . " erstellt einen neuen Administrator: " . $admin->nick . "(" . $admin->id . ").");
    ...
    if ($request->request->has('edit_submit')) {
        if ($request->request->get('user_nick') != "") {
            ...
            if ($request->request->get('user_password') != "") {
                ...
                Log::add(LogFacility::ADMIN, Log::INFO, "Der Administrator " . $cu->nick . " ändert das Passwort des Administrators " . $adminUser->nick . "(" . $adminUser->id . ").");
            }
            ...
            if ($request->request->has('tfa_remove')) {
                $adminUser->tfaSecret = "";
                Log::add(LogFacility::ADMIN, Log::INFO, "Der Administrator " . $cu->nick . " deaktiviert die Zwei-Faktor-Authentifizierung des Administrators " . $adminUser->nick . "(" . $adminUser->id . ").");
            }
            ...
            $twig->addGlobal('successMessage', "Gespeichert!");
            Log::add(LogFacility::ADMIN, Log::INFO, "Der Administrator " . $cu->nick . " ändert die Daten des Administrators " . $adminUser->nick . " (ID: " . $adminUser->id . ").");
        } else {
            echo "Nick nicht angegeben!<br/><br/>";
        }
    }
    ...
    if ($request->query->has('del') && $request->query->getInt('del') > 0 && $request->query->getInt('del') != $cu->id) {
        ...
        if ($adminUser != null && $adminUserRepo->remove($adminUser)) {
            Log::add(LogFacility::ADMIN, Log::INFO, "Der Administrator " . $cu->nick . " löscht den Administrator " . $adminUser->nick . " (ID: " . $adminUser->id . ").");
