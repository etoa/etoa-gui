<?PHP
	...
	if (isset($_POST['create']))
	{
		...
		catch (Exception $e)
		{
			...
			// Show message
			echo MessageBox::error("", "Beim Ausf&uuml;hren des Backup-Befehls trat ein Fehler auf: ".$e->getMessage());
		}
	}
	...
	elseif (isset($_GET['action']) && $_GET['action']=="backuprestore" && $_GET['date']!="")
	{
		...
		try 
		{
			...
			try 
			{
				...
				// Show message
				echo MessageBox::ok("", "Das Backup ".$restorePoint." wurde wiederhergestellt und es wurde eine Sicherungskopie der vorherigen Daten angelegt!");
			}
			catch (Exception $e) 
			{
				// Release mutex
				...
				// Show message
				echo MessageBox::error("", "Beim Ausf&uuml;hren des Restore-Befehls trat ein Fehler auf! ".$e->getMessage());
			}
		}
		catch (Exception $e)
		{
	...
	if (isset($_POST['submit_changes']))
	{
		...
		echo MessageBox::ok("", "Einstellungen gespeichert");
	}
	...
	if ($dir != null)
	{
		...
		echo MessageBox::error("", "Das Backupverzeichnis wurde nicht gefunden!");
	}
?>
