...
function searchUser($val,$field_id='user_nick',$box_id='citybox',$separator=";")
{
	...
	if (mysql_num_rows($res)>0)
	{
		...
		{
			$nCount++;
			$sOut .= "<a href=\"#\" onclick=\"javascript:document.getElementById('".$field_id."').value='".htmlentities($arr[0],ENT_QUOTES,'UTF-8')."';document.getElementById('".$box_id."').style.display = 'none';\">".htmlentities($arr[0],ENT_QUOTES,'UTF-8')."</a>";
			$sLastHit = $arr[0];
		}
	}
