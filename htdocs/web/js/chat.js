...
jquery__WEBPACK_IMPORTED_MODULE_0___default()(function () {
  // Chat event handlers
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#cform').submit(function () {
    sendChat();
    return false;
  });
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#logoutButton').click(function () {
    logoutFromChat();
  });
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#ctext').keyup(function (event) {
    handleCTextKey(event);
  }); // add/remove unread messages indicator on scrolling

  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#chatitems').scroll(function () {
    updateViewed();
  });
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#usercount').click(fetchUserList); // gives focus to the input field.

  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#ctext').focus(); // Start polling

  if (jquery__WEBPACK_IMPORTED_MODULE_0___default()('#chatitems').length != 0) {
    poll(true);
    updateUserList();
  } else {
    logOut();
  ...
  function resizeUi() {
    var h = jquery__WEBPACK_IMPORTED_MODULE_0___default()(window).height();
    var w = jquery__WEBPACK_IMPORTED_MODULE_0___default()(window).width();
    jquery__WEBPACK_IMPORTED_MODULE_0___default()("#chatitems").css('height', jquery__WEBPACK_IMPORTED_MODULE_0___default()("#chatcontainer").height() - 20);
  }
...
function poll(doLoop) {
  ajaxResponderRequest('chat_poll', {
    ...
    if (data.cmd) {
      if (data.cmd == 'lo') logOut();else if (data.cmd == 'ki') kicked(data.msg);else if (data.cmd == 'li') {
        logIn(data.msg);
        new UpdateThread().syncUpdate(data.out, data.lastId);
      } else if (data.cmd == 'bn') banned(data.msg);else if (data.cmd == 'up') new UpdateThread().syncUpdate(data.out, data.lastId);else msgFail('Serverfehler: notProtocol');
    }

    if (doLoop) {
      chatPollTimeout = setTimeout(function () {
        poll(true);
      }, chatPollDelayMilliseconds);
    }

    hideLoading();
  }, function (err) {
    localMsg('Verbindung zum Server fehlgeschlagen (' + err + '). Möglicherweise' + ' ist der Server überlastet. Versuche den Chat neu zu laden.');
  });
}
...
function logOut() {
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('body').empty().append('<p>Sie sind ausgeloggt</p>');
  exitChat();
...
function updateUserList() {
  ajaxResponderRequest('chat_userlist', null, function (data) {
    if (data.length == 0) {
      ...
      jquery__WEBPACK_IMPORTED_MODULE_0___default()('#usercount').html('' + data.length + ' User');
    }
  }, function (err) {
...
function fetchUserList() {
  ajaxResponderRequest('chat_userlist', null, function (data) {
    if (data.length == 0) {
      ...
      var elem = jquery__WEBPACK_IMPORTED_MODULE_0___default()('<div>');
      elem.addClass('serverMessage');
      elem.append(jquery__WEBPACK_IMPORTED_MODULE_0___default()('<div>').text("User im Chat:")); // Append each user

      jquery__WEBPACK_IMPORTED_MODULE_0___default().each(data, function (key, val) {
        elem.append(jquery__WEBPACK_IMPORTED_MODULE_0___default()('<div>').text(" ").append(jquery__WEBPACK_IMPORTED_MODULE_0___default()('<a>').attr('href', 'index.php?page=userinfo&id=' + val.id).attr('target', 'main').text(val.nick)));
      });
      jquery__WEBPACK_IMPORTED_MODULE_0___default()('#chatitems').append(elem);
      jquery__WEBPACK_IMPORTED_MODULE_0___default()('#ctext').focus();
      scrollDown();
    }
  }, function (err) {
    msgFail('Serverfehler: ' + err);
...
function msgFail(rtext) {
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#loadingMessage').empty().append(rtext);
}

function msgClear() {
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#loadingMessage').empty();
}

function showLoading() {
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#loadingAnimation img').show();
}

function hideLoading() {
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#loadingAnimation img').fadeOut();
...
function scrollDown() {
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('#chatitems').animate({
    scrollTop: jquery__WEBPACK_IMPORTED_MODULE_0___default()('#chatitems').prop('scrollHeight')
  }, "fast");
...
function sendChat() {
  showLoading();
  var input = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#ctext');
  ...
  if (ctext) {
    ...
    ajaxResponderRequest('chat_push', {
      ...
      if (data.cmd) {
        if (data.cmd == 'nu' || data.cmd == 'nl') logOut();else if (data.cmd == 'aa') adminMsg(data.msg);else if (data.cmd == 'de') msgFail('Doppelter Text');else if (data.cmd == 'bl') banlist(data.list);else msgFail('Serverfehler: notProtocol');
      } else {
      ...
      jquery__WEBPACK_IMPORTED_MODULE_0___default()('#ctext').val('').focus();
    });
  } else {
    msgFail('Kein Text');
  }
...
function handleCTextKey(event) {
  ...
  if (event.which == 38) {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#ctext').val(msgHistory[msgHistoryIdx]);
...
function updateViewed(ev) {
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('.unviewed').filter(function () {
    return !isInvisible(jquery__WEBPACK_IMPORTED_MODULE_0___default()(this));
  }).removeClass('unviewed').addClass('viewed');
  unviewed = jquery__WEBPACK_IMPORTED_MODULE_0___default()('.unviewed').length; // update invisible count

  if (unviewed > 0) {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#unread').show().html(unviewed + '&nbsp;&darr;');
  } else {
    jquery__WEBPACK_IMPORTED_MODULE_0___default()('#unread').hide();
  }
...
function UpdateThread() {
  ...
  this.id = ++UpdateThread.ThreadCounter; // unsynchronized update() function
  // only call via mutex
  ...
  this.update = function (out, lastId) {
    ...
    if (out) {
      if (isNaN(lastId)) {
        ...
        var doScroll = jquery__WEBPACK_IMPORTED_MODULE_0___default()('#chatitems').prop('scrollHeight') - jquery__WEBPACK_IMPORTED_MODULE_0___default()('#chatitems').prop('clientHeight') - jquery__WEBPACK_IMPORTED_MODULE_0___default()('#chatitems').prop('scrollTop') < 1;
        var added = 0; // appends the messages to the chat window

        jquery__WEBPACK_IMPORTED_MODULE_0___default().each(out, function (key, val) {
          if (jquery__WEBPACK_IMPORTED_MODULE_0___default()('#chatmsg_' + val.id).length == 0) {
            var elem = jquery__WEBPACK_IMPORTED_MODULE_0___default()('<div>');
            ...
            if (val.userId == "0") {
              ...
              if (val.admin) {
                switch (val.admin) {
                  //yellow star
                  case "1":
                    elem.append(jquery__WEBPACK_IMPORTED_MODULE_0___default()('<img>').attr('src', 'images/star_y.gif').attr('alt', 'Admin').attr('title', 'Admin'));
                    break;

                  case "2":
                    elem.append(jquery__WEBPACK_IMPORTED_MODULE_0___default()('<img>').attr('src', 'images/star_g.gif').attr('alt', 'Chat-Moderator').attr('title', 'Chat-Moderator'));
                    break;

                  case "3":
                    elem.append(jquery__WEBPACK_IMPORTED_MODULE_0___default()('<img>').attr('src', 'images/star_r.gif').attr('alt', 'Entwickler').attr('title', 'Entwickler'));
                    break;

                  case "4":
                    elem.append(jquery__WEBPACK_IMPORTED_MODULE_0___default()('<img>').attr('src', 'images/star_green.gif').attr('alt', 'Leiter Team Community').attr('title', 'Leiter Team Community'));
                    break;

                  case "5":
                    elem.append(jquery__WEBPACK_IMPORTED_MODULE_0___default()('<img>').attr('src', 'images/star_c.gif').attr('alt', 'Entwickler').attr('title', 'Entwickler'));
                    break;
                }
              }

              var link = jquery__WEBPACK_IMPORTED_MODULE_0___default()('<a>').attr('href', 'index.php?page=userinfo&id=' + val.userId).attr('target', 'main').text(val.nick);
...
function UpdateMutex(threadObj, o, lid) {
  ...
  this.attempt = function (startMutexObject) {
    ...
    for (var j = startMutexObject; j; j = UpdateMutex.next(j.threadObject.id)) {
      if (j.enter || j.number && (j.number < this.number || j.number == this.number && j.threadObject.id < this.threadObject.id)) {
      ...
      this.threadObject.update(this.out, this.lastId); // now the exclusive access can be released

      this.number = 0; // and we can delete this object
  ...
  this.out = o;
  this.lastId = lid;
  UpdateMutex.Wait[this.threadObject.id] = this; // set number to current timestamp

  this.enter = true;
  this.number = new Date().getTime();
  this.enter = false; // auto-start attempt to acquire mutex
