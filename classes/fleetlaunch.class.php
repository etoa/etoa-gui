							...
							if($this->targetEntity->ownerId()>0)
							{
								if (!$this->targetEntity->ownerHoliday() || $ai->allowOnHoliday())
								{
									if ($ai->attitude() > 1)
									{
										if (!$battleban)
										{
											if($this->sourceEntity->ownerAlliance() && $this->sourceEntity->owner->alliance->checkWar($this->targetEntity->ownerAlliance()) || ($this->userId==$this->sourceEntity->lastUserCheck() || !($this->sourceEntity->ownerPoints()*USER_ATTACK_PERCENTAGE>$this->targetEntity->ownerPoints()  || $this->sourceEntity->ownerPoints()/USER_ATTACK_PERCENTAGE < $this->targetEntity->ownerPoints() ) 
