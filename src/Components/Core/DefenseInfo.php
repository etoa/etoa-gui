...
#[AsLiveComponent(template: 'components/defense_info.html.twig')]
class DefenseInfo extends AbstractController
{
    ...
    {
        return !!$this->defenseQueueRepository->searchQueueItems(DefenseQueueSearch::create()->entity($this->getCurrentEntity())->endAfter(time()));
    }
    ...
    {
        $yard = $this->buildingListItemRepository->findOneBy(['entity' => $this->getCurrentEntity(), 'building' => BuildingId::DEFENSE]);
        ...
        if ($need_bonus_level >= 0) {
            return StringUtils::formatPercentString($time_boni_factor) . " durch Stufe " . $yard->getCurrentLevel();
        } else {
            return "Stufe " . $this->configurationService->getInt('build_time_boni_waffenfabrik') . " erforderlich!";
        }
    }
    ...
    {
        $building = $this->buildingListItemRepository->findOneBy(['entity' => $this->getCurrentEntity(), 'building' => BuildingId::DEFENSE->value]);

        $cancelMinLevel = $this->configurationService->getInt('defqueue_cancel_min_level');
        $cancelEnd = $this->configurationService->getFloat('defqueue_cancel_end');
        $cancelFactor = $this->configurationService->getFloat('defqueue_cancel_factor');
        $cancelStart = $this->configurationService->getFloat('defqueue_cancel_start');

        // Faktor der zurückerstatteten Ressourcen bei einem Abbruch des Auftrags berechnen
        if ($building->getCurrentLevel() >= $cancelMinLevel) {
            $cancel_res_factor = min($cancelEnd, $cancelStart + (($building->getCurrentLevel() - $cancelMinLevel) * $cancelFactor));
        } else {
            $cancel_res_factor = 0;
        }
