...
class AllianceStatsImageGenerator
{
    ...
    public function create(Alliance $alliance, int $width = 600): void
    {
        $height = $width / 3 * 2;
        ...
        $pointEntries = $this->alliancePointsRepository->getPoints($alliance, self::DETAIL_LIMIT * 6, $start, $end);
        if (count($pointEntries) > 0) {
            ...
            foreach ($pointEntries as $entry) {
                if ($last_update === 0) $last_update = $entry->getTimestamp();
                if ($cnt === 0) {
                    $points[$entry->getTimestamp()] = $entry->getPoints();
                    $pmax = max($pmax, $entry->getPoints());
                }
                $cnt++;
            ...
            imagestring($im, self::FONT_SIZE, (int)(self::PADDING / 3), (int)(self::PADDING / 3), "Statistiken von " . $alliance->toString() . ", Rang " . $alliance->getCurrentRank() . ", letztes Update: " . date("d.m.Y H:i", $last_update), $black);
