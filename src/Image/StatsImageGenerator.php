...
class StatsImageGenerator
{
    ...
    public function create(User $user, int $width = 600, ?int $start = null, ?int $end = null): void
    {
        $height = $width / 3 * 2;
        ...
        $pointsEntries = $this->userPointsRepository->getPoints($user, self::DETAIL_LIMIT * 6, $start, $end);
        ...
        if (count($pointsEntries) > 0) {
            ...
            imagestring($im, self::FONT_SIZE, (int)(self::PADDING / 3), (int)(self::PADDING / 3), "Statistiken von " . $user->getNick() . ", Rang " . $user->getRank() . ", letztes Update: " . date("d.m.Y H:i", $last_update), $black);
