...
function battle($fleet_id,$planet_id)
{
        ...
        if (mysql_num_rows($psres)>0)
        {
            while ($psarr=mysql_fetch_array($psres))
            {
            	...
            	if($psarr['special_ship']==1)
            	{
                    ...
                    "ship_exp"=>$sfarr['shiplist_special_ship_exp'],
                    "ship_level"=>$sfarr['shiplist_special_ship_level'],
                    "ship_bonus_weapon"=>$sfarr['shiplist_special_ship_bonus_weapon'],
                    "ship_bonus_structure"=>$sfarr['shiplist_special_ship_bonus_structure'],
                    "ship_bonus_shield"=>$sfarr['shiplist_special_ship_bonus_shield'],
                    "ship_bonus_heal"=>$sfarr['shiplist_special_ship_bonus_heal'],
                    "ship_bonus_capacity"=>$sfarr['shiplist_special_ship_bonus_capacity'],
                    "ship_bonus_speed"=>$sfarr['shiplist_special_ship_bonus_speed'],
                    "ship_bonus_pilots"=>$sfarr['shiplist_special_ship_bonus_pilots'],
                    "ship_bonus_tarn"=>$sfarr['shiplist_special_ship_bonus_tarn'],
                    "ship_bonus_antrax"=>$sfarr['shiplist_special_ship_bonus_antrax'],
                    "ship_bonus_forsteal"=>$sfarr['shiplist_special_ship_bonus_forsteal'],
                    "ship_bonus_destroy"=>$sfarr['shiplist_special_ship_bonus_build_destroy'],
                    "ship_bonus_antrax_food"=>$sfarr['shiplist_special_ship_bonus_antrax_food'],
                    "ship_bonus_deactivade"=>$sfarr['shiplist_special_ship_bonus_deactivade'])
                    );
            ...
            FROM
                ...
                ".$db_table['shiplist']." AS sl
            WHERE
                sl.shiplist_planet_id='$planet_id'
                AND sl.shiplist_user_id='$user_d_id'
                AND sl.shiplist_ship_id=s.ship_id
                AND sl.shiplist_count>0
                AND s.ship_heal>'0'");
	...
	//
        ...
        WHERE
        	...
        	AND (techlist_tech_id=".STRUCTURE_TECH_ID." OR techlist_tech_id=".SHIELD_TECH_ID." OR techlist_tech_id=".WEAPON_TECH_ID." OR techlist_tech_id=".REGENA_TECH_ID.")
   		;");
	...
	//
        ...
        dbquery("UPDATE ".$db_table['deflist']." SET deflist_count=0 WHERE deflist_planet_id='$planet_id'; ");
        dbquery("UPDATE ".$db_table['shiplist']." SET shiplist_count=0 WHERE shiplist_planet_id='$planet_id'; ");
        dbquery("DELETE FROM ".$db_table['fleet_ships']." WHERE fs_fleet_id='$fleet_id';");
	...
	//
        ...
        if ($count_d==0 && $count_a>0)
        {
			...
			foreach ($ships_a as $id=>$data)
			{
				$msg.="\nname=".$data['name']."\ncnt=".$data['cnt']."\nnew_cnt=".$data['new_cnt']."\n";
			}
            //Stellt die Schiffe wieder her (att)
            if (count($ships_a_db))
            {
                foreach ($ships_a_db as $dbid=>$dbitem)
                {
                    dbquery("
                    INSERT INTO
            	...
            	foreach ($special_ships_a as $id=>$data)
            	{
            		echo "<br>new_cnt=".$data['new_cnt']."<br>";
            		echo "<br>cnt=".$data['cnt']."<br>";
            		echo "<br>ship_bonus_weapon=".$data['ship_bonus_weapon']."<br>";
            		echo "<br>ship_exp=".$data['ship_exp']."<br>";
            	}            
            //Stellt die Spezialschiffe wieder her (att)
            if (count($special_ships_a_db))
            {
            	
            	
                foreach ($special_ships_a_db as $dbid=>$dbitem)
                {
					echo "<br>wapon=".$special_ships_a[$dbid]['ship_bonus_weapon']."<br>
					ships_a= ".$special_ships_a[$dbid]."<br>dbid=".$dbid."<br>";
                    dbquery("
                    INSERT INTO
                    ".$db_table['fleet_ships']."
                        (fs_fleet_id,
                    ...
                    VALUES
                    	...
                    	$dbitem,
                    	'1',
                    	'".$special_ships_a[$dbid]['ship_level']."',
                    	'".($special_ships_a[$dbid]['ship_exp']+$new_exp_a_init)."',
                    	'".$special_ships_a[$dbid]['ship_bonus_weapon']."',
                    	'".$special_ships_a[$dbid]['ship_bonus_structure']."',
                    	'".$special_ships_a[$dbid]['ship_bonus_shield']."',
                    	'".$special_ships_a[$dbid]['ship_bonus_heal']."',
                    	'".$special_ships_a[$dbid]['ship_bonus_capacity']."',
                    	'".$special_ships_a[$dbid]['ship_bonus_speed']."',
                    	'".$special_ships_a[$dbid]['ship_bonus_pilots']."',
                    	'".$special_ships_a[$dbid]['ship_bonus_tarn']."',
                    	'".$special_ships_a[$dbid]['ship_bonus_antrax']."',
                    	'".$special_ships_a[$dbid]['ship_bonus_forsteal']."',
                    	'".$special_ships_a[$dbid]['ship_bonus_destroy']."',
                    	'".$special_ships_a[$dbid]['ship_bonus_antrax_food']."',
                    	'".$special_ships_a[$dbid]['ship_bonus_deactivade']."');");
                    ...
                    UPDATE
                    	".$db_table['shiplist']."
                    SET
                        shiplist_special_ship_exp=shiplist_special_ship_exp+".$new_exp_a_init."
                    WHERE
                    	shiplist_user_id='$user_a_id'
                    	AND shiplist_planet_id='".$fleetarr['fleet_planet_from']."'
                    	AND shiplist_ship_id='$dbid';");
            ...
            if (count($defs_db))
            {
                foreach ($defs_db as $dbid=>$dbitem)
                {
                    if (mysql_num_rows(dbquery("SELECT deflist_def_id FROM ".$db_table['deflist']." WHERE deflist_planet_id='$planet_id' AND deflist_def_id='$dbid';"))>0)
                    {
                        dbquery("
                        UPDATE
                        	".$db_table['deflist']."
                        SET
                        	deflist_count=$dbitem
                        WHERE
                        	deflist_planet_id='$planet_id'
                        	AND deflist_def_id='$dbid';");
                    }
                    else
                    {
                        dbquery("
            ...
            foreach ($special_ships_d_db as $dbid=>$dbitem)
            {
                dbquery("
                UPDATE
                    ".$db_table['shiplist']."
            ...
            SET
                ...
                fleet_res_fuel=".$raid_r_to_ship[3].",
                fleet_res_food=".$raid_r_to_ship[4]."
            WHERE
            	fleet_id='$fleet_id';");
        ...
        elseif ($count_a==0 && $count_d>0)
        {
            ...
            dbquery("DELETE FROM ".$db_table['fleet']." WHERE fleet_id='$fleet_id';");
            ...
            if (count($ships_d_db))
            {
                foreach ($ships_d_db as $dbid=>$dbitem)
                {
                    if (mysql_num_rows(dbquery("SELECT shiplist_ship_id FROM ".$db_table['shiplist']." WHERE shiplist_planet_id='$planet_id' AND shiplist_ship_id='$dbid';"))>0)
                    {
                        dbquery("
                        UPDATE
                        	".$db_table['shiplist']."
                        SET
                        	shiplist_count=$dbitem
                        WHERE
            ...
            if (count($special_ships_d_db))
            {
                foreach ($special_ships_d_db as $dbid=>$dbitem)
                {
                    if (mysql_num_rows(dbquery("SELECT shiplist_ship_id FROM ".$db_table['shiplist']." WHERE shiplist_planet_id='$planet_id' AND shiplist_ship_id='$dbid' AND shiplist_special_ship=1;"))>0)
                    {
                        dbquery("
                        UPDATE
                        	".$db_table['shiplist']."
                        ...
                        UPDATE
                        	".$db_table['shiplist']."
                        SET
                            shiplist_special_ship_exp=shiplist_special_ship_exp+$new_exp_d_init
                        WHERE
                            shiplist_user_id='$user_d_id'
                            AND shiplist_planet_id='$planet_id'
                            AND shiplist_ship_id='$dbid';");
                    }
                    else
                    {
        ...
        else
        {
            ...
            else
            {
                ...
                //Spezialschiffe wieder herstellen (att)
                if (count($special_ships_a_db))
                {
                    foreach ($special_ships_a_db as $dbid=>$dbitem)
                    {
                ...
                //Spezialschiffe wieder herstellen (def)
                if (count($special_ships_d_db))
                {
                    foreach ($special_ships_d_db as $dbid=>$dbitem)
                    {
