<?PHP
	...
	// Rohstoffe auf Null setzen falls negativ
	if ($_POST['fleet_res_metal']<0) $_POST['fleet_res_metal']=0;
	if ($_POST['fleet_res_crystal']<0) $_POST['fleet_res_crystal']=0;
	if ($_POST['fleet_res_plastic']<0) $_POST['fleet_res_plastic']=0;
	if ($_POST['fleet_res_fuel']<0) $_POST['fleet_res_fuel']=0;
	if ($_POST['fleet_res_food']<0) $_POST['fleet_res_food']=0;
	if ($_POST['fleet_res_people']<0) $_POST['fleet_res_people']=0;
	...
	if (mysql_num_rows($pres)>0)
	{
		...
		//Bewohner überprüfen
		$people_total_arr=mysql_fetch_array(dbquery("SELECT planet_people FROM ".$db_table['planets']." WHERE planet_id=".$c->id." AND planet_user_id=".$_SESSION[ROUNDID]['user']['id'].""));
		...
		if ($people_free>=0)
		{
			...
			if ($c->res->fuel>=$_SESSION['haven']['fleettotal']['flight_costs'])
			{
				...
				if ($c->res->food>=$_SESSION['haven']['fleettotal']['flight_food'])
				{
					...
					{
						$res = dbquery("SELECT shiplist_count FROM ".$db_table['shiplist']." WHERE shiplist_ship_id=$id AND shiplist_user_id=".$_SESSION[ROUNDID]['user']['id']." AND shiplist_planet_id='".$c->id."';");
		  		...
		  		if ($scnt>0)
		  		{
		  			...
		  			// Gefakte Schiffe bei Fakeangriff auswählen
						...
						foreach ($_SESSION['haven']['ships'] as $id=>$cnt)
						{
							...
							dbquery("UPDATE ".$db_table['shiplist']." SET shiplist_count=shiplist_count-$cnt WHERE shiplist_ship_id=$id AND shiplist_user_id=".$_SESSION[ROUNDID]['user']['id']." AND shiplist_planet_id='".$c->id."';");
						}

						// Rohstoffe vom Planetenkonto abziehen
						dbquery("
						UPDATE 
							".$db_table['planets']." 
						SET
	...
	{
		$_SESSION['haven']=Null;
		echo "<b>Fehler:</b> Ziel existiert nicht!<br/><br/>";
		echo "<input type=\"button\" name=\"back\" onClick=\"document.location='?page=$page';\" value=\"Neue Flottenauswahl\" />";
	}
?>
