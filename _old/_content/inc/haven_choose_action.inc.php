	...
	if (count($_SESSION['haven']['ships'])>0)
	{
		...
		if (mysql_num_rows($pres)>0)
		{
      	...
      	$p1=0;
			...
			$s = sqrt(pow($dx,2)+pow($dy,2));			// Distanze zwischen den beiden Zellen
			...
			else
			{
				...
				{
					$whres = dbquery("SELECT techlist_current_level FROM ".$db_table['techlist']." WHERE techlist_tech_id=".TECH_WORMHOLE." AND techlist_user_id=".$_SESSION[ROUNDID]['user']['id'].";");
				...
				else
				{
                    ...
                    if ($parr['planet_id']>0)
                    {
                        ...
                        {
                            if ($parr['planet_user_id']==$_SESSION[ROUNDID]['user']['id'])
                    ...
                    // Eigener Planet
                    elseif ($parr['planet_user_id']==$_SESSION[ROUNDID]['user']['id'])
                    {
                    ...
                    elseif ($parr['planet_user_id']>0)
                    {
                        ...
                        WHERE
                            ...
                            OR (alliance_bnd_alliance_id2='".$_SESSION[ROUNDID]['user']['alliance_id']."'
                        ...
                        elseif ( ($_SESSION[ROUNDID]['user']['points']*USER_ATTACK_PERCENTAGE<=$uarr['user_points'] && $_SESSION[ROUNDID]['user']['points']/USER_ATTACK_PERCENTAGE>=$uarr['user_points']) || $uarr['user_last_online']<INACTIVE_TIME || ($uarr['user_blocked_from']>0 && $uarr['user_blocked_from']<time() && $uarr['user_blocked_to']>time()) || mysql_num_rows($war)>0)
                        {
                            $mutli_res = dbquery("SELECT
                          		 user_multi_multi_user_id
                            FROM
                            	".$db_table['user_multi']."
                            WHERE
                            	user_multi_user_id='".$_SESSION[ROUNDID]['user']['id']."';");
                            $mutli_arr = mysql_fetch_array($mutli_res);

                        	//Handel unter "Multis" nicht möglich
                        	if($mutli_arr['user_multi_multi_user_id']!=$parr['planet_user_id'])
                        	{
                            	echo "<input type=\"radio\" name=\"fleet_action\" value=\"to\" checked=\"checked\"> Waren senden<br/>";
                            	echo "<input type=\"radio\" name=\"fleet_action\" value=\"so\"> Ausspionieren<br/>";
                            }
                            else
                            {
                        ...
                        else
                        {
                        	...
                        	{
                            	echo "<input type=\"radio\" name=\"fleet_action\" value=\"to\" checked=\"checked\"> Waren senden<br/>";
                            	echo "Anf&auml;ngerschutz aktiv! Die Punkte des Users m&uuml;ssen zwischen ".(USER_ATTACK_PERCENTAGE*100)."% und ".(100/USER_ATTACK_PERCENTAGE)."% von deinen Punkten liegen!<br/>";
                            }
                            else
                            {
                            	echo "Handel unter \"Multis\" nicht gestattet<br>";
                    ...
                    elseif ($parr['planet_user_id']==0)
                    {
                        ...
                        {
                            if (mysql_num_rows(dbquery("SELECT planet_id FROM ".$db_table['planets']." WHERE planet_user_id='".$_SESSION[ROUNDID]['user']['id']."';"))<=USER_MAX_PLANETS)
                    ...
                    if ($launchable)
                    {
                        ...
                        echo "<tr><td class=\"tbltitle\" width=\"40%\" rowspan=\"5\" valign=\"top\">Waren mitnehmen:<br> (max. ".nf($_SESSION['haven']['fleet']['res_capacity'])." t)<br/>";
                        echo "<a href=\"javascript:;\" onclick=\"fulload(".strlen($_SESSION['haven']['fleet']['res_capacity']).");\">Alles einladen</a>";
                        echo "</td>";
                        echo "<td class=\"tbldata\" width=\"60%\"><input type=\"text\" name=\"fleet_res_metal\" value=\"0\" size=\"6\" maxlength=\"".strlen($_SESSION['haven']['fleet']['res_capacity'])."\" onKeyPress=\"return nurZahlen(event)\">&nbsp;t ".$rsc['metal']."</td></tr>";
                        echo "<td class=\"tbldata\" width=\"60%\"><input type=\"text\" name=\"fleet_res_crystal\" value=\"0\" size=\"6\" maxlength=\"".strlen($_SESSION['haven']['fleet']['res_capacity'])."\" onKeyPress=\"return nurZahlen(event)\">&nbsp;t ".$rsc['crystal']."</td></tr>";
                        echo "<td class=\"tbldata\" width=\"60%\"><input type=\"text\" name=\"fleet_res_plastic\" value=\"0\" size=\"6\" maxlength=\"".strlen($_SESSION['haven']['fleet']['res_capacity'])."\" onKeyPress=\"return nurZahlen(event)\">&nbsp;t ".$rsc['plastic']."</td></tr>";
                        echo "<td class=\"tbldata\" width=\"60%\"><input type=\"text\" name=\"fleet_res_fuel\" value=\"0\" size=\"6\" maxlength=\"".strlen($_SESSION['haven']['fleet']['res_capacity'])."\" onKeyPress=\"return nurZahlen(event)\">&nbsp;t ".$rsc['fuel']."</td></tr>";
                        echo "<td class=\"tbldata\" width=\"60%\"><input type=\"text\" name=\"fleet_res_food\" value=\"0\" size=\"6\" maxlength=\"".strlen($_SESSION['haven']['fleet']['res_capacity'])."\" onKeyPress=\"return nurZahlen(event)\">&nbsp;t ".$rsc['food']."</td></tr>";
                        if ($_SESSION['haven']['fleet']['people_capacity']>0)
                        {
                            echo "<tr><td class=\"tbltitle\" width=\"40%\" rowspan=\"1\" valign=\"top\">Bewohner mitnehmen: (max. ".nf($_SESSION['haven']['fleet']['people_capacity']).")<br/></td>";
                            echo "<td class=\"tbldata\" width=\"60%\"><input type=\"text\" name=\"fleet_res_people\" value=\"0\" size=\"6\" maxlength=\"".strlen($_SESSION['haven']['fleet']['people_capacity'])."\" onKeyPress=\"return nurZahlen(event)\">&nbsp; Bewohner</td></tr>";
                        }
                    }
                    infobox_end(1);
