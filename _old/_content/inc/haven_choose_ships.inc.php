<?PHP
	...
	// Rassenspeed laden
	$rres=dbquery("SELECT race_f_fleettime FROM ".$db_table['races']." WHERE race_id=".$_SESSION[ROUNDID]['user']['race_id'].";");
	...
	if (mysql_num_rows($res)>0)
	{
        ...
        while ($arr = mysql_fetch_array($res))
        {
            ...
            WHERE
                ...
                AND techlist.techlist_user_id=".$_SESSION[ROUNDID]['user']['id']."
            GROUP BY
            ...
            echo "<td class=\"tbldata\">".$arr['ship_name']."</td>";
            ...
            echo "</td>";
            echo "<td class=\"tbldata\" width=\"110\"><input type=\"text\" name=\"ship_count[".$arr['ship_id']."]\" size=\"10\" maxlength=\"".strlen($arr['shiplist_count'])."\" value=\"$val\"  title=\"Anzahl Schiffe eingeben, die mitfliegen sollen\" onKeyPress=\"return nurZahlen(event)\" tabindex=\"".$tabulator."\"></td></tr>\n";
            $tabulator++;
        }
        infobox_end(1);

		echo "Wichtig: Schnellere Schiffe nehmen im Flottenverband automatisch die Geschwindigkeit des langsamsten Schiffes an, sie brauchen daf&uuml;r aber auch entsprechend weniger Treibstoff!<br/><br/>";
		...
		{
            echo "<input type=\"submit\" name=\"submit_shipselection_all\" value=\"Alle Schiffe ausw&auml;hlen &gt;&gt;&gt;\" title=\"Klicke hier um alle Schiffe auszuw&auml;hlen\" tabindex=\"".($tabulator+2)."\">&nbsp;";
			echo "<input type=\"submit\" name=\"submit_shipselection\" value=\"Weiter zur Zielauswahl &gt;&gt;&gt;\" title=\"Wenn du die Schiffe ausgew&auml;hlt hast, klicke hier um das Ziel auszuw&auml;hlen\" tabindex=\"".($tabulator+1)."\" /> &nbsp;";
