<?PHP
	...
	if ($_POST['cancel']!="" && checker_verify())
	{
		...
		WHERE
			fleet_id='".$fleet_id."'
			AND fleet_user_id='".$_SESSION[ROUNDID]['user']['id']."'
		...
		if (mysql_num_rows($res)>0)
		{
			...
			add_log(13,"Der Spieler [b]".$_SESSION[ROUNDID]['user']['nick']."[/b] bricht den Flug seiner Flotte [b]".$fleet_id."[/b] ab",time());
		}
		else
	...
	if (mysql_num_rows($res)>0)
	{
		...
		echo "<tr><td class=\"tbltitle\" style=\"width:200px;\">Auftrag:</td><td class=\"tbldata\">".get_fleet_action($arr['fleet_action'])."</td></tr>";
		...
		echo "<tr><td class=\"tbltitle\" style=\"width:100px;\">".RES_METAL."</td><td class=\"tbldata\">".nf($arr['fleet_res_metal'])." t</td></tr>";
