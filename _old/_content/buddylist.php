<?PHP
	...
	if ($_GET['allow']>0)
	{
		...
		FROM
			...
			".$db_table['users']."
		WHERE
			buddylist.bl_user_id=".$_GET['allow']."
			AND buddylist.bl_user_id=users.user_id
			AND buddylist.bl_buddy_id=".$_SESSION[ROUNDID]['user']['id'].";");
		if (mysql_num_rows($res)>0)
		{
			$arr=mysql_fetch_array($res);
			dbquery("UPDATE ".$db_table['buddylist']." SET bl_allow=1 WHERE bl_user_id=".$_GET['allow']." AND bl_buddy_id=".$_SESSION[ROUNDID]['user']['id'].";");
			dbquery("DELETE FROM ".$db_table['messages']." WHERE message_user_to=".$_SESSION[ROUNDID]['user']['id']." AND message_user_from=0 AND message_subject='Buddylist-Anfrage von ".$arr['user_nick']."';");
	...
	if ($_GET['deny']>0)
	{
		...
		{
			$arr=mysql_fetch_array($res);
			dbquery("DELETE FROM ".$db_table['buddylist']." WHERE bl_user_id=".$_GET['deny']." AND bl_buddy_id=".$_SESSION[ROUNDID]['user']['id'].";");
			dbquery("DELETE FROM ".$db_table['messages']." WHERE message_user_to=".$_SESSION[ROUNDID]['user']['id']." AND message_user_from=0 AND message_subject='Buddylist-Anfrage von ".$arr['user_nick']."';");
	...
	if (($_POST['buddy_nick']!="" && $_POST['submit_buddy']!="") || $_GET['add_id']>0)
	{
		...
		{
			$arr=mysql_fetch_array($res);
			if ($_SESSION[ROUNDID]['user']['id']!=$arr['user_id'])
			{
				if (mysql_num_rows(dbquery("SELECT bl_user_id FROM ".$db_table['buddylist']." WHERE bl_user_id='".$_SESSION[ROUNDID]['user']['id']."' AND bl_buddy_id='".$arr['user_id']."';"))==0)
				{
					dbquery("INSERT INTO ".$db_table['buddylist']." (bl_user_id,bl_buddy_id,bl_allow) VALUES('".$_SESSION[ROUNDID]['user']['id']."','".$arr['user_id']."',0);");
					ok_msg("[b]".$arr['user_nick']."[/b] wurde zu deiner Liste hinzugef&uuml;gt und ihm wurde eine Best&auml;tigungsnachricht gesendet!");
					send_msg($arr['user_id'],5,"Buddylist-Anfrage von ".$_SESSION[ROUNDID]['user']['nick'],"Der Spieler will dich zu seiner Freundesliste hinzuf&uuml;gen. Willst du dies erlauben?\n\n[url ?page=buddylist&allow=".$_SESSION[ROUNDID]['user']['id']."]Erlauben[/url] [url ?page=buddylist&deny=".$_SESSION[ROUNDID]['user']['id']."]Verbieten[/url]");
				}
				else
	...
	if ($_GET['remove']>0)
	{
		dbquery("DELETE FROM ".$db_table['buddylist']." WHERE bl_user_id='".$_SESSION[ROUNDID]['user']['id']."' AND bl_buddy_id='".$_GET['remove']."';");
	}
	...
	{
		dbquery("DELETE FROM ".$db_table['buddylist']." WHERE bl_user_id='".$_GET['removeremote']."' AND bl_buddy_id='".$_SESSION[ROUNDID]['user']['id']."';");
	}
	...
	FROM
        ...
        ".$db_table['planets']."
	WHERE
        buddylist.bl_user_id='".$_SESSION[ROUNDID]['user']['id']."'
        AND buddylist.bl_buddy_id=users.user_id
        AND user_id=planets.planet_user_id
	...
	echo "<form action=\"?page=$page\" method=\"post\"><b>Nick:</b> <input type=\"text\" name=\"buddy_nick\" /> <input type=\"submit\" name=\"submit_buddy\" value=\"Freund hinzuf&uuml;gen\" /></form><br/><br/>";
	...
	FROM
        ...
        ".$db_table['users']."
	WHERE
        buddylist.bl_buddy_id='".$_SESSION[ROUNDID]['user']['id']."'
        AND buddylist.bl_user_id=users.user_id
