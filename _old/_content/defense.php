  ...
  if (mysql_num_rows($werft_res)>0)
  {
      ...
      $def_fields=0;
  		  ...
  		  AND queue_endtime>'".time()."'
				AND queue_user_id='".$_SESSION[ROUNDID]['user']['id']."'
			;");
			if (mysql_num_rows($field_res)>0)
			{
				while($field_arr=mysql_fetch_array($field_res))
			...
			$fields_available = $c->fields+$c->fields_extra-$c->fields_used-$def_fields-$building_fields;		
    	...
    	// Infos anzeigen
    	echo "Es arbeiten <b>".nf($people_working)."</b> Arbeiter in deiner Fabrik.<br/>
    	Der Level der Fabrik ergibt eines Bonusfaktor von ".$time_boni_factor.".<br/>";
    	if ($cancel_res_factor>0)
    	{
    		echo "Bei einem Abbruch eines Auftrags werden ".($cancel_res_factor*100)."% der Ressourcen zur&uuml;ckerstattet (".RES_FOOD." ausgenommen).<br/><br/><br/>";
    	}
    	else
    	{
    		echo "Auftr&auml;ge k&ouml;nnen noch nicht abgebrochen werden, Stufe ".DEFQUEUE_CANCEL_MIN_LEVEL." erforderlich.<br/><br/><br/>";
    	}
	...
	****************************/
			...
			if(count($_POST)>0 && isset($_POST['sort_submit']) && checker_verify())
			{
				dbquery("
				UPDATE
					".$db_table['users']."
				...
				$_SESSION[ROUNDID]['user']['item_order_def']=$_POST['sort_value'];
        $_SESSION[ROUNDID]['user']['item_order_way']=$_POST['sort_way'];	
			}
			...
			echo "<tr>
							<td class=\"tbldata\">
								...
								{		
									...
									if($_SESSION[ROUNDID]['user']['item_order_def']==$value)
									{
					...
					echo "</select>
							...
							<td class=\"tbldata\">
								<select name=\"sort_way\">";
									...
									if($_SESSION[ROUNDID]['user']['item_order_way']=='ASC') echo " selected=\"selected\"";
									echo ">Aufsteigend</option>";
									
									//Absteigend
									echo "<option value=\"DESC\"";
									if($_SESSION[ROUNDID]['user']['item_order_way']=='DESC') echo " selected=\"selected\"";
    	      ...
    	      if($build_cnt > $darr['def_max_count'] && $darr['def_max_count']!=0)
    	    	{
    	      	$build_cnt=$darr['def_max_count'];
						...
						if ($fields_available - $darr['def_fields'] * $build_cnt < 0)
						{
				...
				//Log Speichern
				add_log_game_def($log_text,$_SESSION[ROUNDID]['user']['id'],$_SESSION[ROUNDID]['user']['alliance_id'],$c->id,1,time());					
	...
	*********************/
			if (isset($_GET['cancel']) && $_GET['cancel']>0)
			{
				...
				if (mysql_num_rows($qres)>0)
				{
					...
					//Log Speichern
					add_log_game_def($log_text,$_SESSION[ROUNDID]['user']['id'],$_SESSION[ROUNDID]['user']['alliance_id'],$c->id,0,time());					
				}
			}
    	...
    	ORDER BY
			...
			if (mysql_num_rows($qres)>0)
			{
				...
				while ($qarr = mysql_fetch_array($qres))
				{
					...
					{
						$obj_cnt=$qarr['queue_cnt'];
					}
					echo "<tr>";
					echo "<td class=\"tbldata\" id=\"objcount\">".$obj_cnt."</td>";
					echo "<td class=\"tbldata\">".$qarr['def_name']."</td>";
					echo "<td class=\"tbldata\">".df($absolute_starttime)."</td>";
					echo "<td class=\"tbldata\">".df($absolute_starttime+$qarr['queue_endtime']-$qarr['queue_starttime'])."</td>";
					if ($first)
					{
						echo "<td class=\"tbldata\" id=\"objtime\">-</td>";
						$first=false;
					}
					else
					{
						echo "<td class=\"tbldata\" id=\"objtime\">".tf($qarr['queue_endtime']-$qarr['queue_starttime'])."</td>";
					}
					echo "<td class=\"tbldata\" id=\"cancel\"><a href=\"?page=$page&amp;cancel=".$qarr['queue_id']."\" onclick=\"return confirm('Soll dieser Auftrag wirklich abgebrochen werden?');\">Abbrechen</a></td>
					</tr>";
					...
					?>
						<script type="text/javascript">
							...
							function setCountdown()
							{
								if (cnt>0)
								{
									t = Math.floor(obj_rem / 3600 / 24);
									h = Math.floor(obj_rem / 3600);
									m = Math.floor((obj_rem-(h*3600))/60);
									s = Math.floor((obj_rem-(h*3600)-(m*60)));
									obj_rem_formated = h+"h "+m+"m "+s+"s";
									document.getElementById('objtime').innerHTML=obj_rem_formated;
									obj_rem--;
									if (obj_rem==0)
									{
										cnt--;
										obj_rem=obj_time;
										document.getElementById('objcount').innerHTML=cnt;
									}
									setTimeout("setCountdown()",1000);
								}
								else
								{
									document.getElementById('objtime').firstChild.nodeValEe='-';
									document.location='?page=<?PHP echo $page;?>';
								}
							}
							cnt = <?PHP echo $obj_cnt;?>;
							obj_time = <?PHP echo $obj_time;?>;
							obj_rem = <?PHP echo $obj_t_remaining;?>;
							setCountdown();
						</script>
					<?PHP
				}
	...
	***********************/
			...
			if (mysql_num_rows($cres)>0)
			{
				while($carr=mysql_fetch_array($cres))
				{
					...
					$order="def_".$_SESSION[ROUNDID]['user']['item_order_def']." ".$_SESSION[ROUNDID]['user']['item_order_way']."";
  	        ...
  	        AND deflist_user_id='".$_SESSION[ROUNDID]['user']['id']."'
   				WHERE
    				...
    				AND def_show='1'
    				AND (def_race_id='0' OR def_race_id='".$_SESSION[ROUNDID]['user']['race_id']."')
    			ORDER BY
    				".$order.";");
					if (mysql_num_rows($dres)>0)
					{
						//Einfache Ansicht
						if ($_SESSION[ROUNDID]['user']['item_show']!='full')
						{
							echo '<tr>
			    			      	...
			    			      	<tr>
			    			      	  ...
			    			      	  else
			    			      	  {
			    			      	      echo "<th class=\"tbltitle\" height=\"30\">In Aufrag geben:</th>
			    			      	      			<td class=\"tbldata\"><input type=\"text\" value=\"0\" name=\"build_count[".$darr['def_id']."]\" size=\"2\" maxlength=\"5\" ".tm("",$tm_cnt)." tabindex=\"".$tabulator."\" /> St&uuml;ck</td>";
			    			      	  }
  			      	  echo "</tr>";
  			      	  echo "<tr>
  			      	...
  			      	else
  			      	{
			      			...
			      			echo "<tr>
  			      			...
  			      			echo "<td class=\"tbltitle\" width=\"30%\">
  			      							<span style=\"font-weight:500\">".$darr['def_name']."<br/>
  			      							Gebaut:</span> ".nf($darr['deflist_count'])."
  			      						</td>
			  			      			...
			  			      			{
			  			      			    echo "<td class=\"tbldata\"><input type=\"text\" value=\"0\" name=\"build_count[".$darr['def_id']."]\" size=\"2\" maxlength=\"5\" ".tm("",$tm_cnt)." tabindex=\"".$tabulator."\"/></td></tr>";
			  			      			}
