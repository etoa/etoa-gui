<?PHP
	...
	include_once ("inc/get_fleet_action.php");
	...
	if (mysql_num_rows($fres)>0)
	{
		...
		while ($farr = mysql_fetch_array($fres))
		{
			...
			echo "<td class=\"tbldata\"><a href=\"?page=fleetinfo&id=".$farr['fleet_id']."\">".get_fleet_action($farr['fleet_action'])."</a><br/>";
	...
	$spy_tech_level = get_spy_tech($_SESSION[ROUNDID]['user']['id']);
	$fres = dbquery("
	SELECT
      f.fleet_id,
      f.fleet_user_id,
	...
	FROM
      ...
      AND p.planet_user_id='".$_SESSION[ROUNDID]['user']['id']."'
      ...
      else
      {
        ...
        while ($fshiparr = mysql_fetch_array($fshipres))
        {
			...
			if (SPY_TECH_SHOW_ACTION<=$spy_tech_level)
			{
				...
				$ship_action = get_fleet_action($farr['fleet_action']);
			}
			...
			if (time() - $farr['fleet_landtime'] - ($farr['fleet_launchtime'] - $farr['fleet_landtime']) * (1-(0.1*$tarned))>0 && $show_tarn==1)
			{
				if ($header!=1) 
				{
					echo "<tr><td class=\"tbltitle\">Start / Ziel</td><td class=\"tbltitle\">Startzeit / Landezeit</td><td class=\"tbltitle\">Gesinnung</td><td class=\"tbltitle\">Spieler</td></tr>";
					$header=1;
				}
				$deal=1;
				echo "<tr><td class=\"tbldata\" style=\"".$style."\">".coords_format2($farr['fleet_planet_from'],1,$act_color)."<br/>";
				echo "".coords_format2($farr['fleet_planet_to'],1,$act_color)."</td>";
				echo "<td class=\"tbldata\" style=\"".$style."\">".date("d.m.y, H:i:s",$farr['fleet_launchtime'])."<br/>";
				echo "".date("d.m.y H:i:s",$farr['fleet_landtime'])."</td>";
				echo "<td class=\"tbldata\" style=\"".$style."\">";
				echo $action;
				echo "</td>";
				if($farr['fleet_action']=='mto' OR $farr['fleet_action']=='mpo')
				{
					echo "<td class=\"tbldata\">Markt</td></tr>";
				}
				else
				{
					echo "<td class=\"tbldata\" style=\"".$style."\"><a href=\"?page=messages&mode=new&message_user_to=".$farr['fleet_user_id']."\">".get_user_nick($farr['fleet_user_id'])."</a></td></tr>";
				}
