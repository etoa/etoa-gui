<?PHP
  ...
  if (mysql_num_rows($werft_res)>0)
  {
  	...
  	define('CURRENT_SHIPYARD_LEVEL',$werft_arr['buildlist_current_level']);
    	...
    	// Infos anzeigen
    	echo "Es arbeiten <b>".nf($people_working)."</b> Arbeiter in deiner Werft.<br/>
    	Der Level der Werft ergibt eines Bonusfaktor von ".$time_boni_factor.".<br/>";
    	...
    	{
    		echo "Auftr&auml;ge k&ouml;nnen noch nicht abgebrochen werden, Stufe ".SHIPQUEUE_CANCEL_MIN_LEVEL." erforderlich.<br/><br/><br/>";
    	}
	...
	****************************/
			...
			if(count($_POST)>0 && isset($_POST['sort_submit']) && checker_verify())
			{
				...
				$_SESSION[ROUNDID]['user']['item_order_ship']=$_POST['sort_value'];
			...
			echo "<tr>
							<td class=\"tbldata\">
								...
								{		
									...
									if($_SESSION[ROUNDID]['user']['item_order_ship']==$value)
									{
	...
	****************************/
			...
			if(count($_POST)>0 && isset($_POST['submit']) && checker_verify())
			{
    	      ...
    	      if($build_cnt > $sarr['ship_max_count'] && $sarr['ship_max_count']!=0)
				...
				//Log Speichern
				add_log_game_ship($log_text,$_SESSION[ROUNDID]['user']['id'],$_SESSION[ROUNDID]['user']['alliance_id'],$c->id,1,time());					
	...
	*********************/
			if (isset($_GET['cancel']) && $_GET['cancel']>0)
			{
				...
				if (mysql_num_rows($qres)>0)
				{
					...
					//Log Speichern
					add_log_game_ship($log_text,$_SESSION[ROUNDID]['user']['id'],$_SESSION[ROUNDID]['user']['alliance_id'],$c->id,0,time());					
				}
			}
	...
	***********************/
			...
			if (mysql_num_rows($cres)>0)
			{
				while($carr=mysql_fetch_array($cres))
				{
					...
					$order="ship_".$_SESSION[ROUNDID]['user']['item_order_ship']." ".$_SESSION[ROUNDID]['user']['item_order_way']."";
				    			      	...
				    			      	<tr>
				    			      	  ...
				    			      	  else
				    			      	  {
				    			      	      echo "<th class=\"tbltitle\" height=\"30\">In Aufrag geben:</th>
				    			      	      			<td class=\"tbldata\"><input type=\"text\" value=\"0\" name=\"build_count[".$sarr['ship_id']."]\" size=\"2\" maxlength=\"5\" ".tm("",$tm_cnt)." tabindex=\"".$tabulator."\" /> St&uuml;ck</td>";
				    			      	  }
    			      	  echo "</tr>";
    			      	  echo "<tr>
    			      	...
    			      	else
    			      	{
  			      			...
  			      			echo "<tr>
	  			      			...
	  			      			echo "<td class=\"tbltitle\" width=\"30%\">
				  			      			...
				  			      			{
				  			      			    echo "<td class=\"tbldata\"><input type=\"text\" value=\"0\" name=\"build_count[".$sarr['ship_id']."]\" size=\"2\" maxlength=\"5\" ".tm("",$tm_cnt)." tabindex=\"".$tabulator."\"/></td></tr>";
				  			      			}
