<?PHP
	...
	if ($_GET['id']!="")
	{
		...
		//überprüfung, ob der user dieses Schiff auch wirklich schon gebaut hat
		$check_res = dbquery("
        SELECT
        	shiplist.shiplist_ship_id
        FROM
        ...
        WHERE
			...
			AND shiplist.shiplist_ship_id='".$_GET['id']."'
			AND shiplist.shiplist_special_ship='1'
			AND shiplist.shiplist_planet_id='".$c->id."'
			AND shiplist.shiplist_count>0
			AND (ships.ship_race_id='0' OR ships.ship_race_id='".$_SESSION[ROUNDID]['user']['race_id']."');");
		...
		if(mysql_num_rows($check_res)>0)
		{
			...
			if($level-$init_level>0 && ($arr['special_ship_max_level']>$init_level || $arr['special_ship_max_level']==0))
			{
				...
				echo "<input type=\"submit\" class=\"button\" name=\"submit_upgrade\" value=\"Gewähltes Upgrade duchführen\" />";
			}
			echo "</form>";
