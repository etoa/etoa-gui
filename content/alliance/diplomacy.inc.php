		...
		echo "<h2>Diplomatie</h2>";
			...
			elseif (isset($_GET['view']) && $_GET['view']>0)
			{
				...
				if (mysql_num_rows($res)>0)
				{
					...
					if ($arr['a1id']==$s['user']['alliance_id'])
					{
						$opId = $arr['a2id'];
						$opName = $arr['a2name'];
					}
			...
			else
			{
				...
				if ($_POST['sbmit_new_bnd']!="" && checker_verify())
				{
					...
					else
					{
						...
						send_msg($arr['alliance_founder_id'],MSG_ALLYMAIL_CAT,"Bündnisanfrage","Die Allianz [b][".$alliances[$s['user']['alliance_id']]['tag']."] ".$alliances[$s['user']['alliance_id']]['name']."[/b] fragt euch für ein Bündnis an.\n
						[b]Text:[/b] ".addslashes($_POST['alliance_bnd_text'])."\n 
						Geschrieben von [b]".$s['user']['nick']."[/b].\n Gehe auf die [page=alliance]Allianzseite[/page] um die Anfrage zu bearbeiten!");
					}
				}
				...
				if ($_POST['sbmit_new_war']!="" && checker_verify())
				{
					...
					else
					{
						...
						send_msg($arr['alliance_founder_id'],MSG_ALLYMAIL_CAT,"Kriegserklärung","Die Allianz [b][".$alliances[$s['user']['alliance_id']]['tag']."] ".$alliances[$s['user']['alliance_id']]['name']."[/b] erklärt euch den Krieg!\n
						Die Kriegserklärung wurde von [b]".$s['user']['nick']."[/b] geschrieben.\n Geh auf die Allianzseite für mehr Details!");
					}
				}
	          ...
	          $arr=mysql_fetch_array($res);
	   				send_msg($arr['alliance_founder_id'],MSG_ALLYMAIL_CAT,"Anfrage zurückgenommen","Die Allianz [b][".$alliances[$s['user']['alliance_id']]['tag']."] ".$alliances[$s['user']['alliance_id']]['name']."[/b] hat ihre Büdnisanfrage wieder zurückgezogen.");
				...
				if (isset($_POST['pact_accept']))
				{
					...
					if(mysql_num_rows($res)>0)
					{
						...
						add_alliance_history($s['user']['alliance_id'],$text);
				...
				if (isset($_POST['pact_reject']))
				{
					...
					if(mysql_num_rows($res)>0)
					{
						...
						// Logt die Absage
						add_alliance_history($s['user']['alliance_id'],"Die Bündnisanfrage [b]".$arr['alliance_bnd_name']."[/b] der Allianz [b][".$arr['a2tag']."] ".$arr['a2name']."[/b] wird abgelehnt!");
				...
				if (mysql_num_rows($bres)>0)
				{
					...
					{
						if ($barr['alliance_bnd_alliance_id1']==$s['user']['alliance_id'])
						{
