<?PHP
		...
		if ($_POST['editmemberssubmit']!="" && checker_verify())
		{
			if (count($_POST['user_alliance_rank_id'])>0)
			{
				foreach ($_POST['user_alliance_rank_id'] as $uid=>$rid)
				{
					...
					{
						add_alliance_history($s['user']['alliance_id'],"Der Spieler [b]".get_user_nick($uid)."[/b] bekommt den Rang [b]".$rank[$rid]."[/b].");
		...
		// Gründer wechseln
		if ($_GET['setfounder']>0 && $isFounder && $s['user']['id']!=$_GET['setfounder'])
		{
			...
			if (mysql_num_rows($ures)>0)
			{
				...
				add_alliance_history($s['user']['alliance_id'],"Der Spieler [b]".get_user_nick($_GET['setfounder'])."[/b] wird vom Spieler [b]".$s['user']['nick']."[/b] zum Gründer befördert.");
				add_log(5,"Der Spieler [b]".get_user_nick($_GET['setfounder'])."[/b] wird vom Spieler [b]".$s['user']['nick']."[/b] zum Gründer befördert.",time());
		...
		if (intval($_GET['kickuser'])>0 && checker_verify())
		{
			...
			WHERE
                        ...
                        AND alliances.alliance_id='".$s['user']['alliance_id']."';");
			if (mysql_num_rows($res))
			{
				...
				add_alliance_history($s['user']['alliance_id'],"Der Spieler [b]".$uarr['user_nick']."[/b] wurde von [b]".$s['user']['nick']."[/b] aus der Allianz ausgeschlossen!");
				add_log(5,"Der Spieler [b]".$uarr['user_nick']."[/b] wurde von [b]".$s['user']['nick']."[/b] aus der Allianz [b][".$arr['alliance_tag']."] ".$arr['alliance_name']."[/b] ausgeschlossen!",time());
			}
			else
      ...
      if ($num>0)
      {
          ...
          echo "<td class=\"tbldata\">-</td>";
			...
			if ($isFounder && $s['user']['id']!=$uarr['user_id'])
			...
			if ($s['user']['id']!=$uarr['user_id'] && $uarr['user_id']!=$arr['alliance_founder_id'])
