		...
		*/
		...
		if (uRes)
		{
			...
			if (uSize > 0)
			{
				...
				if(uRow["planet_user_id"]==fleet_["fleet_user_id"])
				{
				...
				else
				{
					...
					functions::sendMsg((int)fleet_["fleet_user_id"],SHIP_MISC_MSG_CAT_ID,"Landung nicht möglich",msg);
				...
				if (uRes)
				{
					...
					if (uSize > 0)
					{
						...
						if ((int)uRow["cnt"] >= config.nget("user_max_planets",0))
						{
							...
							functions::send0sg((int)fleet_["fleet_user_id"],SHIP_MISC_MSG_CAT_ID,"Landung nicht möglich",msg);
						...
						{
							//Planet zurücksetzen
							resetPlanet((int)fleet_["fleet_target_to"]); //ToDo
							...
							functions::sendMsg((int)fleet_["fleet_user_id"],SHIP_MISC_MSG_CAT_ID,"Planet kolonialisiert",msg);
						}
					}
				}
			}
