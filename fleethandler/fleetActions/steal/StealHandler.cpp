			...
			if (fsRes) {
				...
				if (fsSize > 0) {
					...
					if (this->shipCnt > 0) {
						...
						query << "	techlist ";
						query << "WHERE ";
						query << "	techlist_user_id='" << f->getUserId() << "' ";
						...
						query << "	techlist ";
						query << "WHERE ";
						query << "	techlist_user_id='" << f->getEntityToUserId() << "' ";
						query << "	AND techlist_tech_id='" << config.idget("SPY_TECH_ID") << "';";
						tRes = query.store();
						query.reset();
						...
						if (this->one <= this->two) {
							...
							query << "			AND def.techlist_user_id='" << f->getEntityToUserId() << "' ";
							query << "			AND att.techlist_user_id=" << f->getUserId() << " ";
							...
							if (techRes) {
								...
								if (techSize > 0) {
									...
									functions::sendMsg(f->getUserId(),config.idget("SHIP_WAR_MSG_CAT_ID"),"Spionageangriff",text);
									functions::sendMsg(f->getEntityToUserId(),config.idget("SHIP_WAR_MSG_CAT_ID"),"Spionageangriff",text);
						...
						/** if spy attack failed **/
							...
							text += " verübt.";
							functions::sendMsg(f->getUserId(),config.idget("SHIP_WAR_MSG_CAT_ID"),"Spionageangriff erfolglos",text);
							functions::sendMsg(f->getEntityToUserId(),config.idget("SHIP_WAR_MSG_CAT_ID"),"Spionageangriff erfolglos",text);
						}
					}
					// If no ship with the action was in the fleet
						...
						functions::sendMsg(f->getUserId(),(int)config.idget("SHIP_MISC_MSG_CAT_ID"),"Spionageangriff gescheitert",text);
					}
				}
			}
		}
