...
void battle($fleet_id,$planet_id)
{
    ...
    double structure_a=0;
    double shield_a=0;
    double weapon_a=0;
    double count_a=0;
    doublecount_heal_a=0;
    
    bool alliancesHaveWar = 0;


	//Speichert ID von beiden Kontrahenten
    int userAId = (int)["fleet_user_id"];
    int userDId = getUserIdByPlanet(fleet_["fleet_target_to"]);
    
   	// Kampf abbrechen falls User gleich
    if (userAId==userDId)
    {
	    msgFight = "[b]KAMPFBERICHT[/b]\nvom Planeten ";
		msgFight += formatCoords(fleet_["fleet_target_to"]);
		msgFight += "\n[b]Zeit:[/b] ";
		msgFight += formatTime((int)fleet_["fleet_landtime"]);
		msgFight += "\n\n";
	    msgFight += "[b]Angreifer:[/b] ";
		msgFight += get_user_nick(userAId);
		msgFight += "\n";
	    msgFight += "[b]Verteidiger:[/b] ";
		msgFight += get_user_nick(userDId);
		msgFight += "\n\n";
  	...
  	else if ((int)config.nget("battleban",0)!=0 && (int)config.get("battleban",1)<=time && (int)config.get("battleban",2)>time) //ToDo Timevariable init
	{
  		msgFight = "[b]KAMPFBERICHT[/b]\nvom Planeten ";
		msgFight += functions::formatCoords((int)fleet_["fleet_target_to"]);
		msgFight += "\n[b]Zeit:[/b] ";
		msgFight += functions::formatTime((int)fleet_["fleet_landtime"]);
		msgFight += "\n\n";
	    msgFight += "[b]Angreifer:[/b] ";
		msgFight += functions::getUserNick(userAId);
		msgFight += "\n";
	    msgFight += "[b]Verteidiger:[/b] ";
		msgFight += getUserNick(userDId);
		msgFight += "\n\n";
	...
	msgFight += "\n\n";
    msgFight += "[b]Angreifer:[/b] ";
	msgFight += getUserNick(userAId);
	msgFight += "\n";
    msgFight += "[b]Verteidiger:[/b] ";
	msgFight += getUserNick(userDId);
	msgFight += "\n\n";
    msgFight += "[b]ANGREIFENDE FLOTTE:[/b]\n";
	...
	//
		...
		query << "	AND sl.shiplist_planet_id = f.fleet_planet_from ";
		...
		if (fsRes)
		{
			...
			if (fsSize > 0)
			{
	    		...
	    		for (mysqlpp::Row::size_type i = 0; i<fsSize; i++) 
				{
					...
					{
						shipSteal = (int)fsRow["ship_steal"];
					}
					else
					{
						dontSteal = 1;
